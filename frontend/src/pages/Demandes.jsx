...
const Demandes = () => {
  ...
  useEffect(() => {
    setSelectedUserIds([]);
  }, [filter, typeFilter, cinFilter, dateFilter, sortBy]);
  ...
  const filteredData = useMemo(() => {
    let filtered = demandes.filter(d => {
      ...
      const dateMatch = dateFilter
        ? new Date(d.created_at).toDateString() === new Date(dateFilter).toDateString()
        : true;
      return statusMatch && typeMatch && cinMatch && dateMatch;
    });
    ...
    return filtered;
  }, [demandes, filter, typeFilter, cinFilter, dateFilter, sortBy]);
        ...
        <div className={`flex items-center gap-3 flex-wrap ${isRTL ? 'flex-row-reverse' : ''}`}>
          ...
          {user?.role === 'admin' && (
            <>
              ...
              >
                <option value="newest">{t('demandes.newest')}</option>
                <option value="oldest">{t('demandes.oldest')}</option>
              </select>
