...
const Users = () => {
  ...
  const [sendEmailForm, setSendEmailForm] = useState({ emails: '', title: '', message: '', color: 'orange' });
  ...
  const handleSendEmail = async (e) => {
    ...
    try {
      const res = await api.post('/admin/send-email', sendEmailForm);
      setShowSendEmailModal(false);
      setSendEmailForm({ emails: '', title: '', message: '', color: 'orange' });
      showToast(`${t('users.email_sent')} (${res.data.data.total_emails} ${t('users.total_emails')})`, 'success');
    } catch (err) {
      console.error('Error sending email:', err);
      showToast(t('users.error_sending_email'), 'error');
    } finally {
        ...
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${isRTL ? 'sm:flex-row-reverse' : ''}`}>
          ...
          <div className={`flex flex-wrap items-center gap-3 ${isRTL ? 'flex-row-reverse' : ''}`}>
            ...
            <button
              ...
              className="px-4 py-2.5 text-sm font-semibold rounded-xl bg-green-500 text-white hover:bg-green-600 transition-all duration-200 shadow-md hover:shadow-lg"
            >
          ...
          <div className={`w-full max-w-lg rounded-2xl border shadow-2xl transform transition-all duration-300 ${
            ...
            <form onSubmit={handleSendEmail} className="p-6 space-y-5">
              <div>
                ...
                <textarea
                  value={sendEmailForm.emails}
                  onChange={(e) => setSendEmailForm(prev => ({ ...prev, emails: e.target.value }))}
                  rows={3}
                  placeholder={t('users.emails_placeholder')}
                  className={`w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all duration-200 resize-none ${
                    darkMode
                      ? 'bg-gray-700 border border-gray-600 text-white placeholder-gray-400'
                      : 'bg-gray-50 border border-gray-200 text-gray-900'
                  }`}
              ...
              <div>
                ...
                <input
                  value={sendEmailForm.title}
                  onChange={(e) => setSendEmailForm(prev => ({ ...prev, title: e.target.value }))}
                  type="text"
                  placeholder={t('users.email_title_placeholder')}
                  className={`w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all duration-200 ${
                    darkMode
                      ? 'bg-gray-700 border border-gray-600 text-white placeholder-gray-400'
                      : 'bg-gray-50 border border-gray-200 text-gray-900'
                  }`}
              ...
              <div>
                <label className={`block text-sm font-semibold mb-2 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                  {t('users.email_message')} *
                </label>
                <textarea
                  value={sendEmailForm.message}
                  onChange={(e) => setSendEmailForm(prev => ({ ...prev, message: e.target.value }))}
                  rows={5}
                  placeholder={t('users.email_message_placeholder')}
                  className={`w-full rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/20 focus:border-green-500 transition-all duration-200 resize-none ${
                    darkMode
                      ? 'bg-gray-700 border border-gray-600 text-white placeholder-gray-400'
                      : 'bg-gray-50 border border-gray-200 text-gray-900'
              ...
              <div>
                <div className="flex gap-3">
                  {[
                    { value: 'orange', bg: 'bg-orange-500', hover: 'hover:bg-orange-600' },
                    { value: 'red', bg: 'bg-red-500', hover: 'hover:bg-red-600' },
                    { value: 'green', bg: 'bg-green-500', hover: 'hover:bg-green-600' }
                  ].map((color) => (
                    <button
                      key={color.value}
                      type="button"
                      onClick={() => setSendEmailForm(prev => ({ ...prev, color: color.value }))}
                      className={`w-8 h-8 rounded-full transition-all duration-200 ${
                        sendEmailForm.color === color.value
                          ? `${color.bg} ring-2 ring-offset-2 ring-gray-400 shadow-lg`
